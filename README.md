...
6. **Примеры на практике** — `example` для CRUD с БД, `mistral` для интеграции с внешним API
...
│   │   │
│   │   ├── example/                # Пример домена (ШАБЛОН для CRUD)
│   │   │   ├── __init__.py
...
│   │   │
│   │   └── mistral/                # Пример интеграции с LLM API
│   │       ├── client.py           # HTTP клиент Mistral AI
│   │       ├── tools.py            # Система инструментов агента
...
> **Примечание**: Ниже описан домен `example` — классический CRUD с БД.
> Для интеграции с внешним API см. `mistral/` (без repository, с HTTP-клиентом).
...
> **Совет**: Для CRUD-домена копируйте `example/`. Для интеграции с внешним API — изучите `mistral/` (без repository).
